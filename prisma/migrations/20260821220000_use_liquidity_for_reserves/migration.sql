-- Reserves are funded by every liquid asset. The separate envelope-pool flag
-- represented the same user decision and is no longer needed.
DROP FUNCTION IF EXISTS get_asset_values_for_user_month(TEXT, TIMESTAMP, TEXT);

ALTER TABLE "NetWorthAsset" DROP COLUMN "poolInEnvelopes";
