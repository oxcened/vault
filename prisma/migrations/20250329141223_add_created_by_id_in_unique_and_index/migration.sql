/*
  ...
  - A unique constraint covering the columns `[timestamp,createdById]` on the table `CashFlow` will be added. If there are existing duplicate values, this will fail.
  - A unique constraint covering the columns `[timestamp,createdById]` on the table `NetWorth` will be added. If there are existing duplicate values, this will fail.

*/
-- DropIndex
DROP INDEX "CashFlow_timestamp_idx";

-- DropIndex
DROP INDEX "CashFlow_timestamp_key";

-- DropIndex
DROP INDEX "NetWorth_timestamp_idx";

-- DropIndex
DROP INDEX "NetWorth_timestamp_key";

-- CreateIndex
CREATE INDEX "CashFlow_timestamp_createdById_idx" ON "CashFlow"("timestamp", "createdById");

-- CreateIndex
CREATE UNIQUE INDEX "CashFlow_timestamp_createdById_key" ON "CashFlow"("timestamp", "createdById");

-- CreateIndex
CREATE INDEX "NetWorth_timestamp_createdById_idx" ON "NetWorth"("timestamp", "createdById");

-- CreateIndex
CREATE UNIQUE INDEX "NetWorth_timestamp_createdById_key" ON "NetWorth"("timestamp", "createdById");
  ...
  LOOP
    ...
    income_transactions AS (
        ...
        WHERE "t"."createdById" = inputCreatedById
          AND "t"."timestamp" >= DATE_TRUNC('MONTH', d)
          AND "t"."timestamp" < DATE_TRUNC('MONTH', d) + INTERVAL '1 MONTH'
          AND "t"."type" = 'INCOME'
    ),
    ...
    ELSE
      ...
      ON CONFLICT (timestamp, "createdById") DO UPDATE
