  ...
  WHILE d <= today DO
    ...
    */
    ...
    FROM (
      ...
      WHERE a.createdById COLLATE utf8mb4_unicode_ci = inputCreatedById COLLATE utf8mb4_unicode_ci
    ) AS assets,
    (
      SELECT 
        SUM(
    ...
    IF YEAR(d) = YEAR(today) AND MONTH(d) = MONTH(today) THEN
      ...
      IF ROW_COUNT() = 0 THEN
        ...
        VALUES (UUID(), totalAssets, totalDebts, netValue, today, NOW(), NOW(), inputCreatedById);
      END IF;
    ELSE
      -- For past months, upsert based on the unique timestamp (LAST_DAY of that month).
      INSERT INTO NetWorth (id, totalAssets, totalDebts, netValue, timestamp, createdAt, updatedAt, createdById)
      VALUES (UUID(), totalAssets, totalDebts, netValue, v_currentMonthEnd, NOW(), NOW(), inputCreatedById)
